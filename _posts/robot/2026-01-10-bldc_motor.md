...
### 3.2 변환 프로세스
MCU 내부에서는 센서 값을 받아 3단계에 걸쳐 좌표계를 변환한다.

1.  **Clarke 변환 (3상 → 2상 정지):** $U, V, W$ → $\alpha, \beta$ (여전히 교류)
2.  **Park 변환 (2상 정지 → 2상 회전):** $\alpha, \beta$ → $d, q$ (직류로 변환됨!)
3.  **PID 제어:** 목표 전류와 비교하여 전압 계산.
4.  **Inverse Park/Clarke (역변환):** 계산된 직류 전압($V_d, V_q$)을 다시 모터가 이해할 수 있는 3상 전압($U, V, W$)으로 복구.
...
```mermaid
graph TD
    UserCmd[사용자 명령: 목표 위치] --> PosController
    
    subgraph Position Loop [1. 위치 제어 루프]
    PosController[위치 제어기 P] -->|출력: 목표 속도| VelInput
    FeedbackPos[센서: 현재 각도] -->|피드백| PosController
    end
    
    VelInput --> VelController
    
    subgraph Velocity Loop [2. 속도 제어 루프]
    VelController[속도 제어기 PI] -->|출력: 목표 전류 Iq| CurInput
    FeedbackVel[계산: 현재 속도] -->|피드백| VelController
    end
    
    CurInput --> CurController
    
    subgraph Current Loop [3. 전류 제어 루프 FOC]
    CurController[전류 제어기 PI] -->|출력: 전압 PWM| Motor
    FeedbackCur[센서: 현재 전류] -->|피드백: Park변환된 Id, Iq| CurController
    end
    ...
    style Position Loop fill:#e1f5fe,stroke:#01579b
    style Velocity Loop fill:#e0f2f1,stroke:#004d40
    style Current Loop fill:#fff3e0,stroke:#e65100
```
...
1.  **전류 루프 ($I_d, I_q$):** 전류 센서값 + 엔코더 각도($\theta$)를 이용해 Park 변환한 값을 사용.
