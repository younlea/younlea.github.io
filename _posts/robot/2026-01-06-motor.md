...
<script type="module">
  ...
  document.addEventListener('DOMContentLoaded', function() {
    const blocks = document.querySelectorAll('pre code.language-mermaid, div.language-mermaid pre code');
...
1.  **Phase Currents ($I_a, I_b$) - 전류 센싱**
    ...
    * **용도:** 현재 모터가 내고 있는 **토크(힘)와 자속량**을 계산하는 기초 데이터가 됩니다.

2.  **Rotor Angle ($\theta_{elec}$) - 엔코더 값**
    ...
    * **용도:** **Park 변환**의 핵심 키(Key)입니다. 이 각도를 알아야 회전 좌표계로 변환하여 전류를 직류($I_d, I_q$)처럼 다룰 수 있습니다.
...
**2. 센서리스 vs 센서드 저속 구동 비교 (RCexplained)**
센서가 없는 모터가 저속에서 얼마나 불안정한지(Cogging 현상) 슬로우 모션으로 비교한 영상입니다. 로봇 핸드에 왜 센서가 필요한지 바로 이해할 수 있습니다.
